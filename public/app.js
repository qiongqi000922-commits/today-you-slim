...
const REPLAY_BUILD_VERSION = "20260618-qqonly1";
...
const FACE_MODEL_URL = appUrl("/models/face_landmarker.task");
...
function qqLoginStartUrl({ popup = false, sync = false, mobile = false } = {}) {
...
function openQqAuthorizationPopup({ sync = false, mobile = false } = {}) {
  const popup = window.open(qqLoginStartUrl({ popup: true, sync, mobile }), "qq_login", qqAuthorizationPopupFeatures());
  if (popup) {
...
async function handleQqLoginResult(status) {
  ...
  if (isQqSyncStatus(normalized)) {
    ...
    try {
      ...
      await showSettingsView(state.profile, true, "profile");
      setQqSyncMessage("QQ 头像和昵称已同步。", "success");
    } catch {
      if (state.profile) {
        renderSettings();
...
window.addEventListener("message", (event) => {
  ...
  if (!data || data.type !== "qq-login-result") return;
  handleQqLoginResult(data.status);
});

window.addEventListener("storage", (event) => {
  if (event.key !== QQ_LOGIN_RESULT_KEY) return;
...
window.addEventListener("focus", consumeStoredQqLoginResult);
window.addEventListener("pageshow", consumeStoredQqLoginResult);
...
function renderAccountProfilePanel() {
  ...
  els.editProfileButton.disabled = !state.profile.qq || state.profileSaving;
  els.cancelProfileEditButton.disabled = state.profileSaving;
  els.saveProfileButton.disabled = state.profileSaving || !state.profile.qq;
...
function startProfileEdit() {
  if (!state.profile?.qq) {
    setProfileSettingsMessage("当前账户未绑定 QQ，暂不能编辑个人资料。", "error");
    return;
  }
...
function renderAvatar(element, profile = state.profile) {
  ...
  element.textContent = avatarUrl ? "" : profileInitial(profile);
...
function renderProfileIdentity(profile = state.profile) {
  ...
  if (els.profileName) {
    ...
    els.profileTitle.textContent = `${name} 的减肥记录`;
  }
...
function shouldOfferPasskeyPrompt(profile = state.profile) {
  ...
  return Boolean(profile?.qq && summary.available && summary.promptAvailable && !summary.enabled && webauthnSupported());
}
...
function showPasskeyPrompt() {
  ...
  els.startPasskeyPromptButton.disabled = !webauthnSupported();
...
function renderPasskeyStatus(status = state.passkeyStatus) {
  ...
  const hasQqBinding = status ? Boolean(status.hasQqBinding) : Boolean(profileSummary.available);
  ...
  const statusText = enabled ? `${activeCount} 个` : hasQqBinding ? "未设置" : "需绑定QQ";
  ...
  els.passkeySettingsSummary.textContent = enabled
    ? `已添加 ${activeCount} 个 Passkey，可用于下次直接登录。`
    : hasQqBinding
      ? "当前 QQ 账户还未添加 Passkey。"
      : "必须先通过 QQ 登录并绑定账户。";
  els.passkeyStatusDescription.textContent = hasQqBinding
    ? "Passkey 会绑定当前 QQ 账户，之后可用手机解锁直接登录本站。"
    : "当前账户不是 QQ 绑定账户，不能添加 Passkey。";
  els.passkeyBindingState.textContent = hasQqBinding ? "已绑定 QQ" : "未绑定 QQ";
  els.passkeyBindingHint.textContent = hasQqBinding
    ? `绑定标识：${status?.bindId || state.profile?.qq?.bindId || state.profile?.accountCode || "-"}`
    : "请先在登录页使用 QQ 登录。";
  els.addPasskeyButton.disabled = !hasQqBinding || !webauthnSupported();
  if (!webauthnSupported()) {
    els.passkeyBindingHint.textContent = "当前浏览器不支持 Passkey，请使用手机系统浏览器或新版桌面浏览器。";
  }
    ...
    ? credentials.map((credential) => `
      <div class="passkey-list-item">
        <span>
          ...
          <small>添加于 ${formatConsentTime(credential.createdAt)}${credential.lastUsedAt ? ` · 最近使用 ${formatConsentTime(credential.lastUsedAt)}` : ""}</small>
        </span>
        <button class="passkey-delete-button" type="button" data-passkey-delete="${escapeAttribute(credential.id)}" aria-label="移除 ${escapeAttribute(credential.label || "我的 Passkey")}">${communityTrashIconMarkup()}</button>
      </div>
    `).join("")
    : '<div class="passkey-empty">还没有添加 Passkey。</div>';
}
...
async function saveProfileSettings() {
  if (!state.profile?.qq || state.profileSaving) return;
...
function normalizeSettingsPanel(panel) {
  return ["profile", "privacy", "passkey", "account"].includes(panel) ? panel : "overview";
}
...
function communityWeightTrendMarkup(member) {
  ...
  const bars = visiblePoints.map((point, index) => {
    ...
    if (!Number.isFinite(point.min) || !Number.isFinite(point.max)) {
      return `<line class="community-weight-empty-tick" x1="${x.toFixed(1)}" y1="${(height - padY - 5).toFixed(1)}" x2="${x.toFixed(1)}" y2="${(height - padY).toFixed(1)}" stroke="rgba(31, 122, 90, 0.16)" stroke-width="2.4" stroke-linecap="round" vector-effect="non-scaling-stroke"></line>`;
    }
...
function communityFoodPreviewMarkup(member) {
  ...
  for (let index = 0; index < photos.length; index += 4) {
    groups.push(photos.slice(index, index + 4));
  }
  const groupMarkup = groups.map((group, groupIndex) => {
    const cells = Array.from({ length: 4 }, (_, index) => group[index] || null);
    return `
      <div class="community-food-group" aria-label="饮食照片第 ${groupIndex + 1} 组">
        ${cells.map((photo, index) => photo ? `
          <figure class="community-food-thumb">
            <img src="${escapeAttribute(photo.thumbnailUrl || photo.photoUrl)}" alt="饮食照片 ${groupIndex * 4 + index + 1}" loading="lazy" decoding="async" draggable="false">
          </figure>
  ...
  els.communityGrid.innerHTML = members.map((member) => `
    <article class="community-card" role="button" tabindex="0" data-community-open="${escapeAttribute(member.id)}" aria-label="进入 ${escapeAttribute(communityDisplayName(member))} 的资料">
      ...
      <div class="community-card-body">
        ...
        ${member.latestMood
          ? `<p class="community-card-mood">“${escapeAttribute(member.latestMood)}”</p>`
          : '<p class="community-card-mood is-empty">暂无心情</p>'}
...
function renderCommunityLikeButton(member, variant = "default") {
  ...
  const label = !fullMode ? (hasFullMode() ? accountRestrictedMessage() : "基础模式仅可浏览，不能点赞") : member.isSelf ? "不能给自己的档案点赞" : member.likedByViewer ? "已点赞" : "点赞";
  const pressed = Boolean(member.likedByViewer && !member.isSelf);
  const disabled = member.isSelf || !fullMode;
  ...
  const composer = hasInteractiveFullMode()
    ? `
      <form class="community-comment-form is-open" id="communityCommentForm">
        <input id="communityCommentInput" type="text" maxlength="${TEXT_LIMITS.comment}" autocomplete="off" placeholder="写一句鼓励或回应">
...
function renderCommunityDetailHeading(member = null) {
  ...
  if (els.communityDetailAvatar) {
    ...
    els.communityDetailAvatar.classList.toggle("has-image", Boolean(avatarUrl));
    els.communityDetailAvatar.textContent = avatarUrl ? "" : communityAvatarText(member || { alias: display });
    els.communityDetailAvatar.style.backgroundImage = avatarUrl ? `url("${avatarUrl.replace(/"/g, "%22")}")` : "";
  }
}
...
async function toggleCommunityLike(button) {
  ...
  const currentCount = Number(button.querySelector(".community-like-count")?.textContent || 0);
...
async function shareCommunityMember(button) {
  ...
  const shareData = {
    ...
    text: `我在「今天你瘦了吗?」看到 ${communityDisplayName(member)} 的变化记录。`,
    url: shareUrl.href
  };
  const message = document.querySelector("#communityDetailMessage");
...
function calendarWeightMeta(records) {
  ...
  const latestWeekIndex = (latestDate.getDay() + 6) % 7;
  const endDay = latestDay + (6 - latestWeekIndex) * dayMs;
  const startDay = endDay - 27 * dayMs;
  ...
  return {
    dayMs,
    startDay,
    selectedDay: comparable.at(-1)?.day ?? null,
    maxAbsDelta: Math.max(0.1, ...comparable.map((entry) => Math.abs(entry.delta))),
    metaByDay: new Map(comparable.map((entry) => [entry.day, entry]))
  };
}
...
function weightCalendarInnerMarkup(records, { emptyText = "暂无体重涨跌日历。" } = {}) {
  ...
  const cells = Array.from({ length: 28 }, (_, index) => calendar.startDay + index * calendar.dayMs).map((day) => {
    const date = new Date(day);
    ...
    const alpha = (0.18 + level * 0.52).toFixed(2);
    const color = isUp
      ? `rgba(191, 117, 106, ${alpha})`
      : isDown
        ? `rgba(79, 148, 121, ${alpha})`
...
function renderWeightCalendar() {
  els.weightCalendar.innerHTML = weightCalendarInnerMarkup(state.records, {
...
function renderChart(highlightId = null) {
  const weights = [...weightRecords()].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
  if (weights.length < 2) {
...
function renderFoodHistoryItem(record, { readonly = false } = {}) {
  ...
  return `
    <article class="history-item is-food">
      ...
      ${readonly ? "" : `<button class="delete-record-button" type="button" data-record-id="${escapeAttribute(record.id)}" aria-label="删除这条食物记录">删除</button>`}
    </article>
  `;
}
...
function renderBodyHistoryItem(record, { readonly = false } = {}) {
  return `
    <article class="history-item">
      ${record.thumbnailUrl ? `<img class="history-thumb" src="${record.thumbnailUrl}" alt="记录照片缩略图" loading="lazy" decoding="async">` : `<div class="history-thumb">${record.photoUrl ? "PHOTO" : "WEIGHT"}</div>`}
      <div>
        <p class="history-time">${dateTimeFormat.format(new Date(record.timestamp))}</p>
        <p class="history-note">${record.photoUrl ? "已保存照片" : "仅保存体重"}</p>
        ${record.mood ? `<p class="history-mood">“${escapeAttribute(record.mood)}”</p>` : ""}
      </div>
      <strong class="history-weight">${formatWeight(record.weight)}</strong>
      ${readonly ? "" : `<button class="delete-record-button" type="button" data-record-id="${escapeAttribute(record.id)}" aria-label="删除这条记录">删除</button>`}
    </article>
  `;
}
...
function updateCaptureButtonState() {
  ...
  els.captureButton.textContent = state.faceModelReady ? "拍照" : "模型加载中";
  els.captureButton.disabled = !state.stream || !state.faceModelReady || Boolean(state.capturedPhoto);
  updateMoodAiButtons();
}
...
async function ensureFaceLandmarker() {
  ...
  if (!state.faceModelPromise) {
    state.faceModelPromise = (async () => {
      const { FaceLandmarker, FilesetResolver } = await import(MEDIAPIPE_IMPORT_URL);
...
async function deleteRecord(recordId) {
  ...
  const description = `${dateTimeFormat.format(new Date(record.timestamp))}${Number.isFinite(record.weight) ? `，${formatWeight(record.weight)}` : ""}`;
...
els.loginForm.addEventListener("submit", async (event) => {
  event.preventDefault();
  setMessage(els.loginMessage, "请点击 QQ 登录继续。", "");
});
